...
- 개발 단계(백엔드가 학생 PC에서 실행 중): `pi/.env`의 `BACKEND_URL`은 그 PC의 **로컬 IP**
  ...
  자신을 가리키므로 쓸 수 없습니다.
- **백엔드 호스트 바인딩 필수**: Windows PC에서 백엔드를 띄울 때 반드시 `--host 0.0.0.0`을 지정해야 합니다 (`uvicorn main:app --reload --host 0.0.0.0 --port 8000`). 기본값인 `127.0.0.1`로 바인딩하면 같은 Wi-Fi 망이라도 파이의 접속이 차단됩니다.
...
| 증상 | 원인 | 해결 |
|---|---|---|
| 파이에서 `Connection refused`/timeout | 1) 백엔드가 `127.0.0.1`에 바인딩됨 (기본 실행)<br>2) Windows 방화벽이 인바운드 8000번 포트를 차단 | 1) 백엔드 실행 시 `--host 0.0.0.0` 옵션 지정: `uvicorn main:app --reload --host 0.0.0.0 --port 8000`<br>2) Windows Defender 방화벽 인바운드 규칙에 8000 포트 허용 추가: `netsh advfirewall firewall add rule name="fastapi-dev" dir=in action=allow protocol=TCP localport=8000` |
