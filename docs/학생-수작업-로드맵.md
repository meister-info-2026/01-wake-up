...
> `docs/학생용-설치-및-사용-매뉴얼.md`를 따르고, 이 문서는 수업 중 진행 체크용으로
> 사용합니다.
...
- [ ] 0-3 하네스 구축 프롬프트 1회 입력 → Next.js 스캐폴드 및 `.env.example` 정리 확인
- [ ] *(PowerShell requirements 저장 시 반드시 UTF-8 명시)*:
  ```powershell
...
- [ ] 팀 미팅: `docs/팀-주제-브레인스토밍-워크시트.md`로 주제, 센서, 액추에이터, 트리거, 역할 합의
...
- [ ] *(새 채팅)* hardware-agent 프롬프트 → `pi/main.py` 생성
